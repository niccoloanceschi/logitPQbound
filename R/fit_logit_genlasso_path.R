...
fit_logit_genlasso_path <- function(y, X, D, type=c('NR','BL','PG','PQ'), 
                                    ...
                                    eps=1e-10, gamma=1.0, intercept=FALSE, 
                                    maxiter=1000L, abstol=1e-4, reltol=1e-4, 
                                    etatol=1e-4, verbose=FALSE, freq=10L, 
  ...
  for (k in K:1) {
    ...
    fitk <- fit_logit_genlasso(y=y, X=X, D=D, type=type, 
                               beta_start=beta_start, lambda=lambda[k], 
                               eps=eps, intercept=intercept, maxiter=maxiter, 
...
fit_logit_splasso_path <- function(y, X, D, type=c('NR','BL','PG','PQ'), 
  ...
  for (k in K:1) {
    ...
    pwts <- alpha*active + (1-alpha)*rep(1,m)
