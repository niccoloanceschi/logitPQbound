...
fit_logit_mfvb <- function(y, X, D=diag(ncol(X)), type=c("PQ", "PG", "BL"), 
                           beta_start=NULL, lambda=1.0, eps=1e-10, intercept=FALSE, adaptive=FALSE,
  ...
  fit(y=y, X=X, D=D, beta_start=beta_start, lambda=lambda, eps=eps, 
      intercept=intercept, adaptive=adaptive, solver=solver, maxiter=maxiter, 
...
fit_logit_pqvb <- function(y, X, D=diag(ncol(X)), beta_start=NULL, 
                           lambda=1.0, eps=1e-10, intercept=FALSE, adaptive=FALSE,
                           solver=c("auto", "chol", "smw", "sparse"),
                           maxiter=1000L, abstol=1e-4, reltol=1e-4,
                           verbose=FALSE, freq=10L){
