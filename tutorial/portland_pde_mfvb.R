...
### ---- MCMC ----
{
  ...
  fit_MC <- fit_logit_mcmc(y=y, X=X, D=D, solver="sparse", beta_start=NULL,
                           lambda=lambda, eps=eps, intercept=intercept,
                           maxiter=20000L, verbose=verbose, freq=250L)
...
if (SAVE) {
  filename <- "portland_mfvb_tvd_beta.pdf"
  filepath <- paste(IMGPATH, filename, sep="/")
  height <- 4; width <- 6; zoom <- .9
  pdf(file=filepath, height=zoom*height, width=zoom*width)
  par_old <- par(no.readonly = TRUE)
  par(mar=c(2,4,.5,.5))
  df <- cbind("BL"=1-acc_beta_BL, "PG"=1-acc_beta_PG, "PQ"=1-acc_beta_PQ)
  boxplot(df, col="grey", ylim=c(0,0.5), outline=FALSE, xlab="", ylab="", main="")
  title(xlab="", ylab="Total variation distance", main="")
  par(par_old)
  dev.off()
}
...
if (SAVE) {
  ...
  plt <- plt + theme(axis.title=element_blank(), axis.ticks=element_blank(), 
                     axis.text=element_blank(), strip.text=element_text(size=18),
