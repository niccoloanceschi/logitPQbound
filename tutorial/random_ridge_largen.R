...
data <- get_sim_data(n=200, p=50, ratio=0.7, seed=1234)
...
ctr <- set_ctr_admm(rho=1., precondition=TRUE, 
...
if (SAVE) {
  ...
  filename <- "random_ridge_largen_fit.RDS"
  filepath <- paste(RDSPATH, filename, sep="/")
  saveRDS(fit_list, file=filepath)
}

if (SAVE) {
  filename <- "random_ridge_largen_loglik.pdf"
